...
public abstract class LongAttribute extends NormalAttribute<Long> {


    @Override
    public void setAttribute(PersistentDataHolder target, Long value) {
        ...
        dataContainer.set(key, PersistentDataType.LONG, value);
    }

    @Override
    public Long getAttribute(PersistentDataHolder target) {
        PersistentDataContainer dataContainer = target.getPersistentDataContainer();
        NamespacedKey key = new NamespacedKey(plugin, getEnum().name());
        return dataContainer.get(key, PersistentDataType.LONG);
    }
